...
class MechanismsController < ApplicationController
  ...
  def create
    mech_params = mechanism_params.merge(mechanism_category_id: MechanismSubcategory.find(mechanism_params[:mechanism_subcategory_id]).mechanism_category_id,
  ...
  def edit
    ...
    raise ActionController::RoutingError, 'Страница не найдена' unless @mechanism.present?
  end
  ...
  def show
    @mechanism = Mechanism.where(id: params[:id]).includes(%i[mechanism_subcategory mechanism_category]).first
    raise ActionController::RoutingError, 'Страница не найдена' unless @mechanism.present?
  end
